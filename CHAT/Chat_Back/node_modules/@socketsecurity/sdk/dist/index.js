...
function createRequestBodyForFilepaths(filepaths) {
    ...
    for (const p of filepaths) {
        ...
        requestBody.push(`Content-Disposition: form-data; name="file"; filename="${path.basename(p)}"\n`, `Content-Type: application/octet-stream\n\n`, createReadStream(p), 
...
export class SocketSdk {
    ...
    async getOrgFullScan(orgSlug, fullScanId, file) {
        try {
            const req = https.request(`${this.#baseUrl}orgs/${encodeURIComponent(orgSlug)}/full-scans/${encodeURIComponent(fullScanId)}`, {
                method: 'GET',
                ...this.#reqOptions
            });
            const { 0: res } = await events.once(req, 'response');
